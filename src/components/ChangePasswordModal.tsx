...
export function ChangePasswordModal({ onClose }: { onClose: () => void }) {
  ...
  const lb: React.CSSProperties = { display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-dim)', marginBottom: 6 }
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'var(--overlay)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, boxShadow: 'var(--shadow-lg)', width: '100%', maxWidth: 380, overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontWeight: 500, fontSize: 14.5 }}>เปลี่ยนรหัสผ่าน</span>
        ...
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 13 }}>
          {done ? (
            <div style={{ textAlign: 'center', color: 'var(--ok)', fontWeight: 500, fontSize: 14, padding: '10px 0' }}>✓ เปลี่ยนรหัสผ่านแล้ว</div>
          ) : (
            <>
              ...
              <button onClick={save} disabled={busy || !oldP || !newP || !confirmP}
                style={{ width: '100%', padding: 11, borderRadius: 10, border: 'none', background: 'var(--accent)', color: 'var(--bg)', fontFamily: 'var(--sans)', fontSize: 13.5, fontWeight: 500, cursor: 'pointer', opacity: busy || !oldP || !newP || !confirmP ? 0.6 : 1 }}>
