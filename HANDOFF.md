...
## 9. ธีม + ขนาดตัวอักษร (การแสดงผล)

หน้า **การแสดงผล** (เมนู "อื่น ๆ" · เมนูโปรไฟล์ · ⌘K) ให้ผู้ใช้เลือกเองได้ 2 อย่าง — เก็บใน `localStorage` **ของเครื่อง** ไม่ผูกกับบัญชี

| ค่า | key | ผลที่เกิด |
|---|---|---|
| ธีม (5 แบบ + ตามระบบ) | `facehub_theme` | ตั้ง `data-theme` + `color-scheme` ที่ `<html>` · 'ตามระบบ' ฟัง `prefers-color-scheme` แบบ live |
| ขนาดตัวอักษร (90/100/115/130%) | `facehub_font_scale` | ตั้ง `--app-zoom` ที่ `<html>` → `#root { zoom: var(--app-zoom) }` |

ธีมที่มี (ค่าใน `THEME_IDS` ของ `state.tsx` = ค่าใน `data-theme`):
`light` (ตาม Figma) · `dark` (เทาเข้ม) · `midnight` (น้ำเงินเข้ม) · `sepia` (กระดาษโทนอุ่น) · `contrast` (ขาว-ดำคอนทราสต์สูง) · `hosxp` (หน้าตาโปรแกรมเดสก์ท็อป uniGUI/ExtJS ที่ HOSxP ใช้) · `glass` (กระจกฝ้าแบบ iOS)
⚠️ `glass` ใช้พื้นผิวโปร่ง (rgba) + `backdrop-filter` — เพิ่มกล่อง/แผงใหม่ต้องเติมคลาสลงในลิสต์ `:is(...)` ของธีมนี้ ไม่งั้นโปร่งแต่ไม่เบลอ
⚠️ `hosxp` ทับมากกว่าธีมอื่น: `--r-*` = 0 (มุมเหลี่ยม) · `--sans`/`--mono` = Tahoma · เงาเป็น none แล้วใช้ขอบนูน/บุ๋ม (box-shadow inset) แทน
   เพิ่มกฎอะไรที่ผูกกับมุมมน/เงา/ฟอนต์ ต้องเผื่อธีมนี้ด้วย
`DARK_THEMES` บอกว่าตัวไหนเป็นตระกูลพื้นเข้ม — ใช้ตัดสินใจเรื่องโลโก้ (`isDark` จาก `useApp()`) และกฎ CSS ของปุ่ม/เมนู
เพิ่มธีมใหม่ = เพิ่ม id ใน `THEME_IDS` + บล็อก `:root[data-theme='<id>']` ใน `theme.css` + รายการใน `THEMES` ของ `screens/Display.tsx`

กติกา:
- **ธีมมืดอยู่ที่ `:root[data-theme='dark']` ใน `theme.css`** ทับเฉพาะ token พื้น/ตัวอักษร/เส้น/เงา · สี accent กับสีสถานะใช้ชุดเดียวกับธีมสว่าง (ความหมายของสีต้องไม่เปลี่ยนตามธีม)
- Figma ยังมีแต่โหมดสว่าง — ชุดสีมืดจึงเป็นของฝั่งโค้ด ถ้า Figma เพิ่มโหมดมืดเมื่อไหร่ให้ยึดตามนั้น
- **ห้ามเขียนสีตายตัวในหน้าจอ** (เช่น `rgba(0,0,0,0.1)` หรือ hex) — ธีมมืดจะเพี้ยนทันที รอบนี้แก้ไปแล้ว 4 จุด (Topbar, Overview, HospitalDetail, DesignSystem)
- ขนาดตัวอักษรใช้ `zoom` ไม่ใช่ `rem` เพราะสเกลข้อความในโค้ดเป็น px ตายตัว (inline style) — คูณด้วย CSS อย่างเดียวไม่ครอบคลุม
