...
class HospitalPicker extends StatefulWidget {
  const HospitalPicker({super.key, required this.search, required this.onSelected});
          ...
          children: [
            Center(
              child: Container(
                width: 42,
                height: 4,
                decoration: BoxDecoration(color: Nexus.pLine, borderRadius: BorderRadius.circular(2)),
              ),
            ),
            const SizedBox(height: 16),
            Text('เลือกโรงพยาบาล', style: Nexus.tech(size: 17, weight: FontWeight.w700)),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                prefixIcon: Icon(PhosphorIconsRegular.magnifyingGlass, size: 19, color: Nexus.pDim),
            ...
            Expanded(
              child: _loading
                  ? Center(child: CircularProgressIndicator(color: Nexus.pAccent))
                  : _items.isEmpty
                  ? Center(
                      child: Text(
                        _error ? 'เชื่อมต่อเซิร์ฟเวอร์ไม่ได้ — พิมพ์อีกครั้งเพื่อลองใหม่' : 'ไม่พบโรงพยาบาล',
                  ...
                  : ListView.separated(
                      ...
                      separatorBuilder: (_, __) => Divider(height: 1, color: Nexus.pDivider),
                      itemBuilder: (_, i) {
                        final h = _items[i];
                        return InkWell(
                            ...
                            child: Row(
                              children: [
                                Container(
                                  ...
                                  child: Text(
                                    h.hcode,
                                    style: Nexus.tech(size: 12.5, weight: FontWeight.w700, color: Nexus.pAccent),
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: Text(
                                    ...
                                    style: Nexus.body(size: 14, color: Nexus.pInk),
                                  ),
                                ),
                              ],
                            ),
